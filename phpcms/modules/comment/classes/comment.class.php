...
class comment {
	...
	public function status($commentid, $id, $status = -1) {
		...
		if ($status == 1) {//通过的时候
			...
			$this->check_reward($comment_data['userid'],$id,$comment_data['creat_at']);//评论送奖励
	...
	//评论审核通过后奖励5金币5积分
	public function check_reward($userid,$id,$creat_at){
	    $this->db = pc_base::load_model('member_model');
	    
	    $startdate=strtotime(date("Y-m-d",$creat_at)) ; 
	    $enddate=$startdate+86399;
	    $comment=$this->comment_data_db->count("userid = '$userid' and creat_at >= '$startdate' and creat_at <= '$enddate' and status=1 ");
	    
	    if($comment<='2'){//每天只能奖励两次
	        $updatearr = array('integral'=>'+=5','gold'=>'+=5');
	        $result=$this->db->update($updatearr, array('userid'=>$userid));
	    }
	}
}