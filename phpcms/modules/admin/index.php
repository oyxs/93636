...
class index extends admin {
    ...
    public function new_search(){
        include $this->admin_tpl('admin_search');
    }
    public function new_search_list(){
        ...
        if($typeid==1){//搜索游戏库
            $data=$game->select("`title` like '%$q%'");
        }elseif($typeid==2){//搜索文章
            $data=$content->select("`title` like '%$q%'");
        }elseif($typeid=3){
            $data=$libao->select("`title` like '%$q%'");
    ...
    */
    public function game_api(){
       ...
       if(isset($_POST['id'])&&isset($_POST['game_id'])){
           $id=intval($_POST['id']);
           $game_id=intval($_POST['game_id']);
           $data=array('game_id'=>$game_id);
           $where=array('id'=>$id);
           $result=$this->api_db->update($data,$where);
           if(!empty($result)){
               $data=array('static'=>1);
        ...
        if(!empty($type)){
            $data=$this->api_db ->select($where = array('game_id'=>0));
        }else{
            $data=$this->api_db ->select();
        }

        include $this->admin_tpl('game_api');
