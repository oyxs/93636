...
class index {
	...
	**
	*/
	public function Keyowrd_Url($title){
		$this->db = pc_base::load_model('wx_reply_model');
	 	if(!empty($title)){
	 		$r=$this->db->get_one("`keyword` = '$title' ",'id,keyword,url,type,title,thumb,content','id DESC');
	 		if($r){
	 			$r['url']=str_replace(array("&","%"),array("&amp;","&#37;"),$r['url']);
	 			return $r;	
	 		}else{
	 			return 'no';
	 		}
	...
	public function Story_Url($title){
		$this->db = pc_base::load_model('wx_resource_model');
	 	if(!empty($title)){
	 		$r=$this->db->get_one("`title` = '$title' ",'id,title,wp_url,pwd','id DESC');
	 		if($r){
	 			$r['url_2']="http://m.93636.com/census/1_".$r['id'].".html";
	 			return $r;	
	 		}else{
	 			return 'no';
	 		}
	...
	*/
	public function WX_Redirect() {
		$id = intval($_GET['id']);
	 	if(!empty($id)){
	 		$this->db = pc_base::load_model('wx_resource_model');
	 		$r=$this->db->get_one("`id` = '$id'",'id,title,wp_url,pwd','id DESC');
	 		$code=$r['pwd'];
	 		include template('content','wx_redirect');
	 	}else{
	 		echo 'no';
	 	}
