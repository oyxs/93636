	    ...
	    if (isset($_GET['search'])) {
	        ...
	        if($keyword) {
	            if ($type == '1') {
	                ...
	                $where .= " and `shopid` like '%$keyword%'";
	            } else {
	                $where .= " and `username` like '%$keyword%'";
	            }
	        }
	        if($start_time){
	            $where .= " and FROM_UNIXTIME(cash_time,'%Y-%m-%d') >= '$start_time'";
	        }
	        if($end_time){
	            $where .= " and FROM_UNIXTIME(cash_time,'%Y-%m-%d') <= '$end_time'";
	        }
	    }else{
	        $where='';
	    }
	    $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
	    $cash_arr = $this->cash_db->listinfo($where, 'id DESC', $page, 15);
	    $pages = $this->cash_db->pages;
	    
	    
	    include $this->admin_tpl('member_cash');
	}
