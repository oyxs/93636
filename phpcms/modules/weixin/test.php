...
//         $start = $dates;
//     }
//     if($a == 'user' && $_GET['token']=='nzd9'){   
//         $wechatObj->getusersummary($start,$end);
//     }elseif($a == 'msg' && $_GET['token']=='mynzd9'){
//         $wechatObj->getupstreammsg($start,$end);
//     // }elseif($a == 'article'){
//     //     $end = $_GET['start'];
//     //     $start = $_GET['end'];
//     //     $wechatObj->getarticletotal($start,$end);
//     }
// }else{
...
class wx
{
    ...
    public $appid = "wx32223d1d3b511cb7";
    public $appsecret = "85a865f2ba2013cf91a7390b11f63147";
                    ...
                    switch ($content)
                    {
                        case "V1001_ye":
                            $textTpl = "<xml>
                                ...
                                <Image>
                                <MediaId><![CDATA[QJS4Hd1Q6MpKao47r8OcvWsMnnFQUJAbt30lB4NV-xs]]></MediaId>
                                </Image>
                                </xml>";
                            break;    
                        case "V1001_sh":
                            $textTpl = "<xml>
                                ...
                                <MsgType><![CDATA[image]]></MsgType>
                                <Image>
                                <MediaId><![CDATA[QJS4Hd1Q6MpKao47r8OcvWSA6kgRqwCn52-SS_LdE1o]]></MediaId>
                                </Image>
                                </xml>";
                            break;                      
                        default : 
                            //$content = "您的问题已发送给在线客服，我们将在20分钟内回复您！";
                            break;
                    }
    ...
    public function getusersummary($start,$end){
        ...
        $ch = curl_init('https://api.weixin.qq.com/datacube/getusersummary?access_token=' . $access_token['access_token']);  
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST,"POST");  
        ...
        $ch2 = curl_init('https://api.weixin.qq.com/datacube/getusercumulate?access_token=' . $access_token['access_token']);  
        curl_setopt($ch2, CURLOPT_CUSTOMREQUEST,"POST");  
        curl_setopt($ch2, CURLOPT_POST, 1 );
        curl_setopt($ch2, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch2, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch2, CURLOPT_TIMEOUT, 1);
        $datajson2 = curl_exec ($ch2);
        curl_close($ch2);
        $result2=json_decode($datajson2,true);
        $result=$wx_get->getusersummary($result,$result2);
    }
    //消息数据
    public function getupstreammsg($start,$end){
        $wx_get=new index();
        $access_token = $this -> access_token();
        $data='{ 
                    "begin_date": "'.$start.'", 
        ...
        $ch = curl_init('https://api.weixin.qq.com/datacube/getupstreammsg?access_token=' . $access_token['access_token']);  
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST,"POST");  
        curl_setopt($ch, CURLOPT_POST, 1 );
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $datajson = curl_exec ($ch);
        $result=json_decode($datajson,true);
        $result=$wx_get->getupstreammsg($result);
    }
