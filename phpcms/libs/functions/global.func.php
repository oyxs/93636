 ...
 */
 function trim_script($str) {
	...
	/**
	 * 移动和pc适配,获取当前页面指向pc端的完整URL地址
	 */
	function get_catid_arr(){
		 $sub_catid_arr = array(13,18,19,20,24,25,26,120,121,122,123,124,125,141,152,149,150,151,130,132,133,153,155,156,157,158,159,160,161,171,163,164,165,166,167,168,169,170,174,175,176,177,179,180,178,181,183,184,185,186,187,188,189,190,192,193,194,195,196,197,198,199,201,202,203,204,205,206,207,208); 
		 return $sub_catid_arr; 
		 }
	/**
	 * 移动和pc适配,获取当前页面指向pc端的完整URL地址
	 */
	function new_get_url($catid) {
		//判断当前的栏目是否为有二级栏目栏目id，内页链接数字部分是由$catid.9.$id的组成形式。
		$mydata2 = array(13,18,19,20,24,25,26,120,121,122,123,124,125,141,152,149,150,151,130,132,133,153,155,156,157,158,159,160,161,171,163,164,165,166,167,168,169,170,174,175,176,177,179,180,178,181,183,184,185,186,187,188,189,190,192,193,194,195,196,197,198,199,201,202,203,204,205,206,207,208);
		...
		$id = intval($_GET['id']);//获取当前文章的id
		$don_url = $_SERVER['QUERY_STRING'];//当前动态ur参数
		$show_pos = strpos($don_url,'show');//判断当前的参数中是否有show,来判断是否为内页
		for($i=0;$i<$mydata2_len;$i++){
			if(intval($mydata2[$i])==$catid && is_int($show_pos)){
				$rel_url = explode('/',$relate_url);
				$rel_url_to = $rel_url[1];//取出栏目链接部分
					return $sys_protocal.(($_SERVER['HTTP_HOST']=='m.93636.com') ? 'www.93636.com' : $_SERVER['HTTP_HOST']).'/'.$rel_url_to.'/'.$id.'.html';
				}
			}
		return $sys_protocal.(($_SERVER['HTTP_HOST']=='m.93636.com') ? 'www.93636.com' : $_SERVER['HTTP_HOST']).$relate_url;
		//return $sys_protocal.(isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '').$relate_url;
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			if($curr_page==1) {
				$multipage .= ' <span class="b1">1</span>';
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= ' <span class="b1">'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" id="nextpage" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" id="nextpage" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span class="b1">'.$pages.'</span> <a id="nextpage" href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1" id="nextpage">'.L('next').'</a>';
		}
	}
	return $multipage;
}
 ...
 * @return 分页
 */
function pages_me($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
    if(defined('URLRULE') && $urlrule == '') {
        $urlrule = URLRULE;
        $array = $GLOBALS['URL_ARRAY'];
    } elseif($urlrule == '') {
        $urlrule = url_par_me('page={$page_me}');
    }
    $multipage = '';
    if($num > $perpage) {
        $page = $setpages+1;
 ...
 * URL路径解析，pages 函数的辅助函数
 *
 * @param $par 传入需要解析的变量 默认为，page={$page_me}
 * @param $url URL地址
 * @return URL
 */
function url_par_me($par, $url = '') {
	echo "sss";
    if($url == '') $url = get_url();
    $pos = strpos($url, '?');
    if($pos === false) {
 ...
 */
 ...
 function search_show_url($catid, $id, $typeid=''){
	global $WAP;
	if($typeid=='') {
 ...
 function gid($url){
		if(strpos($url,"&amp")){
		 ...
		 if(strpos($url,"html")){
			 $path = parse_url($url);
			 $sid = $path['path'];
			 $sid = preg_replace('/\D/s', '', $sid);
			 return $sid;
			 }
	 }
//通过单个游戏的文章id，得到更多关于这个游戏其他字段的信息
 function ginfo($id){
	 $game_db = pc_base::load_model('game_model');
	 $sql = "select id,title,downloadtimes,type,size,recommend,bigtu,catid from 93636_game where id=".$id."";
	 $result = $game_db->query($sql);
	 $data = array();
  ...
  //通过单个游戏的id，bigtu字段的图片信息
 function getBigtu($id){
	 $game_db = pc_base::load_model('game_model');
	 $sql = "select id,bigtu from 93636_game where id=".$id."";
	 $result = $game_db->query($sql);
	 $data = array();
	 while($row = mysql_fetch_assoc($result)){
		 array_push($data, $row);
...
//模拟器合集页面，按人气排序，需要给$data里面的downloadtimes赋值后再输出$data
 function fix_data($data){
	 if(!empty($data)){
		 //从游戏模型的数据表中获取所有游戏要用的信息
		 $game_db = pc_base::load_model('game_model');
		 $sql = "select id,downloadtimes from 93636_game";
		 $result = $game_db->query($sql);
		 $Data = array();
		 while($row = mysql_fetch_assoc($result)){
			 array_push($Data, $row);
		 ...
		 foreach($data as $key=>$val){
			foreach($val as $k=>$v){
				foreach($Data as $ky=>$vl){
					foreach($vl as $ko=>$vo){
					if($data[$key]['id']==$Data[$ky]['id']){
						$data[$key]['downloadtimes']=$Data[$ky]['downloadtimes'];
						}	
					} 
		 		}
			} 
...
//通过单个游戏的文章id，得到更多关于这个游戏其他字段的信息
   function ginfoData($id){
	 $game_db = pc_base::load_model('game_model');
	 $sql = "select id,content,pictures from 93636_game_data where id=".$id."";
	 $result = $game_db->query($sql);
	 $data = array();
	 while($row = mysql_fetch_assoc($result)){
		 array_push($data, $row);
...
function get_libao($title){
    $libao_db = pc_base::load_model('libao_model');
    $sql="select * from 93636_libao  where catid='31' and 93636_libao.title like '%$title%' ORDER BY id DESC Limit 0,1";
    $result = $libao_db->query($sql);
    $data = array();
    while($row = mysql_fetch_assoc($result)){
        array_push($data, $row);
    }
   ...
   return $data[0]['url'];
    // {pc:get sql="select * from 93636_libao  where catid='31' and 93636_libao.title like '%$r[title]%' ORDER BY updatetime DESC " return="data" num="1"}

}



function http_post($url, $data_string) {//接口CURL
    $ch = curl_init();
...
function P($vars, $label = '', $return = false) {//打印数组
 ...
 * 判断手机号格式正确
 */
function is_tell($str){
	if(preg_match('/^1([0-9]{9})/',$str)) {
		return true;
	}
}
