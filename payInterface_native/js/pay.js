         ...
         var routeUrl = {
            ...
            $('#auto_center').empty().load(url+' #'+suffix,function(tpl, status, obj){
                ...
                if(suffix === 'orderInfo'){
                    ...
                    var number=$("#number").val();//订单号
                    $("#out_trade_no").val(number);

                    var body=$("#body").val();//订单名称
                    $('input[name=body]').val(body);

                    var total_fee=$("#total_fee").val();//总金额
                    $('input[name=total_fee_pay]').val(total_fee/100);
                    $('input[name=total_fee]').val(total_fee);//真  上线改
